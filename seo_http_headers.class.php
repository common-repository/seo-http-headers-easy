...
class Seo_Http_Head{
    ...
    private function _set_headers($array){
        ...
        if (isset($_ENV['HTTP_IF_MODIFIED_SINCE'])) 
            $IfModifiedSinceE = $this->get_time_unix($_ENV['HTTP_IF_MODIFIED_SINCE']);   
        if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) 
            $IfModifiedSinceS = $this->get_time_unix($_SERVER['HTTP_IF_MODIFIED_SINCE']); 
        if ($lastModifiedUnix <= $IfModifiedSinceE || $lastModifiedUnix <= $IfModifiedSinceS){

            foreach (headers_list() as $header) {
                if(strpos($header, "Expires:")      !== false){header_remove("Expires");}
                if(strpos($header, "X-Pingback:")   !== false){header_remove("X-Pingback");}
                if(strpos($header, "Set-Cookie:")   !== false){header_remove("Set-Cookie");}
                if(strpos($header, "Pragma:")       !== false){header_remove("Pragma");}
                if(strpos($header, "Content-Type:") !== false){header_remove("Content-Type");}
                if(strpos($header, "Cache-Control:")!== false){header_remove("Cache-Control");}
                if(strpos($header, "Pragma:")       !== false){header_remove("Pragma");}
            }
